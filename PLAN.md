...
- [x] **Tanda F1.4 (código completo, PR pendiente de merge+backfill+medición)**: **ingesta de transacciones/IL (fuente NUEVA) + `star_out_flag`** (docs/04 §1.5). Migración 006 `player_transactions` (archivo crudo as-of, idempotente por `mlb_transaction_id`), job `sync_transactions` (endpoint de rango, canario `il_desc_unclassified`), clasificador `il_effect` versionado que reconoce "injured list" (2019+) **y** "disabled list" (pre-2019, para el backfill 2018). `star_out_flag` = cuenta 0/1/2 de los top-2 bateadores establecidos (≥200 PA) en IL as-of `date < event_day` (≤ t-1), ambos mercados, None/NaN nunca 0 fabricado, paridad online/bulk. `closer_available_flag` **diferido a F1.4b** (no guardamos saves/entradas → identidad del cerrador solo proxyeable de forma ruidosa; se decide según el gate). 246 tests. Revisión adversarial de 4 dimensiones: **1 bug de paridad confirmado y corregido** (gate archivo-vivo online vs bulk) + 4 refutados. `feature_version` → `_star_v6` (62/54 columnas). **Nota de entorno**: la verificación necesita el extra `[ml]` o `test_ml.py` (incluida la paridad) se salta entero; y la política de egress bloqueó el smoke en vivo de `statsapi.mlb.com` (el smoke real es el primer dispatch de `sync_transactions`).
...
3. **(HECHO — F1.4, código completo)** Ingesta de transacciones/IL (fuente nueva) + `star_out_flag` (§1.5). Pendiente: merge del PR, aplicar migración 006, backfill histórico de transacciones (`sync_transactions --start-date 2018-03-01`), y train_f1 para medir `star_out_coverage` + el walk-forward vs baseline nuevo (star_out entra a ambos mercados → F5 sí puede moverse; usar baseline nuevo, no el v6). **Revisar el summary del PRIMER dispatch de `sync_transactions` (rango chico) antes del backfill completo**: `il_desc_unclassified` debe ser 0 (si no, drift de taxonomía a revisar) — es el smoke que la política de egress impidió correr localmente.
...
| 2026-07-16 | **Tanda F1.4 (código completo, PR abierto)**: ingesta de transacciones/IL (fuente NUEVA) + `star_out_flag` (§1.5). Migración 006 `player_transactions`, job `sync_transactions`, clasificador `il_effect` versionado (injured list 2019+ y disabled list pre-2019). `star_out_flag` cuenta 0/1/2 de top-2 en IL as-of ≤ t-1, ambos mercados, paridad online/bulk. `closer_available_flag` diferido a F1.4b (no hay saves/entradas para la identidad del cerrador). 246 tests; revisión adversarial de 4 dimensiones (1 bug de paridad confirmado y corregido: gate archivo-vivo online vs bulk). `feature_version` → `_star_v6`. **Pendiente**: merge + migración + backfill + train_f1 para medir (números en el próximo update) |

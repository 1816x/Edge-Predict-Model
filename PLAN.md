...
3. **(Claude — SIGUIENTE TANDA)** Interacción mano del abridor × splits del lineup (ya hay bateo + splits vs mano + `top4_woba_vs_hand`); o `closer_available_flag` (§1.4 restante) y `star_out_flag` (§1.5 restante) — ambos exigen ingerir transacciones/IL (fuente nueva). Evaluar cuál rinde más antes de codear. Alternativa limpia sin fuente nueva: park factors (§1.6) desde los resultados ya archivados.
4. **(Ambos)** Vigilar el email del audit diario (~8:17 AM CST). **Los rojos del 07-10/11/12 ya están diagnosticados y corregidos** (snapshot en el cron diario). Rojo del 07-15 en adelante = diagnóstico antes que nada; si sale verde varios días, el reloj F0 sin huecos por fin corre limpio.
5. **(Automático)** El train_f1 de ~2026-07-24 trae la primera evaluación real del gate (market prior n≥200 en 2026; hoy va en n=71) y coincide con el cierre del reloj F0. Dejar que el archivo crezca.
